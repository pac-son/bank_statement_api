        ...
        for line in lines:
            ...
            if match:
                ...
                if amounts:
                    ...
                    is_inflow = "INFLOW" in upper or "CREDIT" in upper or "+" in line_str
                    is_outflow = "OUTFLOW" in upper or "DEBIT" in upper or "-" in line_str

                    desc = line_no_date
                    for a in amount_matches:
                        desc = desc.replace(a, "")
                    desc = re.sub(r"\s+", " ", desc).strip() or "Moniepoint Transaction"

                    transactions.append({
                        "date": date_str,
                        "description": desc,
                        "debit": amount if is_outflow and not is_inflow else (0.0 if is_inflow else amount),
                        "credit": amount if is_inflow else 0.0,
                        "balance": balance
                    })
